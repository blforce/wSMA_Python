...
/* See Bob's Python code */
      ...
      hByte = (val>>8) & 0xff; 
      lByte = val & 0xff; 
      //printf("val, hByte, lByte = %04x, %04x, %04x\n",val, hByte, lByte);

      data.block[0] = (__u8)len;
      data.block[1] = (__u8)hByte;
      data.block[2] = (__u8)lByte;

      i2c_smbus_access(fd, I2C_SMBUS_WRITE, (__u8)ctl, I2C_SMBUS_I2C_BLOCK_BROKEN, &data);
